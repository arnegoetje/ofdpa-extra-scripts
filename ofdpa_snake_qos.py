...
def main(start_port=1, end_port=32, vlan_id=0):
    ...
    qosIndex_p = 1
    set_qos_port_dscp_trust(qosIndex_p)
    ...
    for in_port in range(start_port, end_port):
        ...
        set_ingress_port_match(in_port, qosIndex_p)
        set_ingress_port_match(in_port + 1, qosIndex_p)

def set_ingress_port_match(in_port, qosIndex):
    # match QoS
    for eth_type in 0x0800,0x86dd:
        ingressPortFlowEntry = ofdpaIngressPortFlowEntry_t()
        ofdpaFlowEntryInit(OFDPA_FLOW_TABLE_ID_INGRESS_PORT, ingressPortFlowEntry)
        ingressPortFlowEntry.flowData.ingressPortFlowEntry.gotoTableId = OFDPA_FLOW_TABLE_ID_PORT_DSCP_TRUST
        ingressPortFlowEntry.flowData.ingressPortFlowEntry.match_criteria.inPort = in_port
        ingressPortFlowEntry.flowData.ingressPortFlowEntry.match_criteria.etherType = eth_type
        ingressPortFlowEntry.flowData.ingressPortFlowEntry.qosIndex = qosIndex

        ofdpaFlowAdd(ingressPortFlowEntry)

def set_qos_port_dscp_trust(qosIndex):
    for dscp_value in range(8):
        dscpTrustFlowEntry = ofdpaDscpTrustFlowEntry_t()
        ofdpaFlowEntryInit(OFDPA_FLOW_TABLE_ID_PORT_DSCP_TRUST, dscpTrustFlowEntry)
        dscpTrustFlowEntry.flowData.dscpTrustFlowEntry.gotoTableId = OFDPA_FLOW_TABLE_ID_VLAN
        dscpTrustFlowEntry.flowData.dscpTrustFlowEntry.match_criteria.qosIndex = qosIndex
        dscpTrustFlowEntry.flowData.dscpTrustFlowEntry.match_criteria.dscpValue = dscp_value
        dscpTrustFlowEntry.flowData.dscpTrustFlowEntry.trafficClass = dscp_value
        dscpTrustFlowEntry.flowData.dscpTrustFlowEntry.color = OFDPA_QOS_GREEN

        ofdpaFlowAdd(dscpTrustFlowEntry)
...
def set_vlan(in_port, vlan_id):
    ...
    vlanFlowEntry.flowData.vlanFlowEntry.newVlanId = OFDPA_VID_PRESENT | (1 if vlan_id == 0 else vlan_id) # use VLAN 1 internally for untagged
